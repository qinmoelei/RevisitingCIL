...
# 这个concat出来两个结果，一个是simplevit出来的最正常的adapater，一个是目前调出来的linear+fc的结果
...
class Learner(BaseLearner):
    def __init__(self, args):
        ...
        _network = SimpleVitNet_linear(args, True)
        _network.convnet = vit_base_patch16_224_in21k_adapter_lora(
            tuning_config=self.args_args
        )
        _network.convnet.out_dim = 768
        self._network_list = []
        self._network_list.append(_network)

        _network = SimpleVitNet(args, True)
        _network.convnet = timm.create_model(
        ...
        _network = SimpleVitNet(args, True)
        _network.convnet = build_promptmodel(
                modelname='vit_base_patch16_224_in21k',
                Prompt_Token_num=args.get("prompt_token_num",10),
                VPT_type=args.get("vpt_type","Shallow"),
            )
        prompt_state_dict = _network.convnet.obtain_prompt()
        _network.convnet.load_prompt(prompt_state_dict)
        _network.convnet.out_dim = 768
        self._network_list.append(_network)
